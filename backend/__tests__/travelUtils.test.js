...
describe('Travel Utilities — Living Cost & Currency', () => {
  ...
  describe('calculateNomadVisaDurationAndOverstayRiskScore', () => {
    it('calculates visa duration and overstay risk correctly when safe', () => {
      const res = calculateNomadVisaDurationAndOverstayRiskScore({
        stayRecords: [{ daysStayed: 30 }, { daysStayed: 20 }],
        targetZoneDaysLimit: 90,
        rollingWindowDays: 180
      });
      expect(res.valid).toBe(true);
      expect(res.totalDaysInZone).toBe(50);
      expect(res.daysRemaining).toBe(40);
      expect(res.overstayDays).toBe(0);
      expect(res.riskTier).toBe('SAFE');
      expect(res.recommendation).toContain('Visa status safe (40 days remaining');
    });

    it('flags OVERSTAY_VIOLATION when total stay exceeds target limit', () => {
      const res = calculateNomadVisaDurationAndOverstayRiskScore({
        stayRecords: [{ daysStayed: 60 }, { daysStayed: 40 }],
        targetZoneDaysLimit: 90,
        dailyOverstayFineUsd: 100
      });
      expect(res.valid).toBe(true);
      expect(res.overstayDays).toBe(10);
      expect(res.estimatedFineRiskUsd).toBe(1000);
      expect(res.riskTier).toBe('OVERSTAY_VIOLATION');
      expect(res.recommendation).toContain('CRITICAL: Overstayed by 10 days!');
    });

    it('returns error for non-array stay records', () => {
  ...
  describe('calculateNomadStartupRunwayExtension', () => {
    it('calculates runway extension correctly', () => {
      const res = calculateNomadStartupRunwayExtension({
        currentMonthlyBurnUsd: 10000,
        currentCashUsd: 100000,
        targetDestinationMonthlyCostUsd: 2000,
        relocationCostUsd: 2000,
        teamSize: 2
      });
      expect(res.valid).toBe(true);
      expect(res.isFeasible).toBe(true);
      expect(res.currentRunwayMonths).toBe(10);
      expect(res.targetMonthlyBurn).toBe(4000);
      expect(res.newRunwayMonths).toBe(24);
      expect(res.runwayExtensionMonths).toBe(14);
      expect(res.recommendation).toContain('Highly Recommended');
    });

    it('returns error for negative cash', () => {
      const res = calculateNomadStartupRunwayExtension({ currentCashUsd: -100 });
      expect(res.valid).toBe(false);
      expect(res.error).toBe('Current cash must be a non-negative number');
    });

    it('handles case where relocation costs exceed cash', () => {
      const res = calculateNomadStartupRunwayExtension({
        currentMonthlyBurnUsd: 10000,
        currentCashUsd: 5000,
        targetDestinationMonthlyCostUsd: 3000,
        relocationCostUsd: 6000,
        teamSize: 1
      });
      expect(res.valid).toBe(true);
  ...
  describe('calculateNomadColivingCostSharingIndex', () => {
    it('calculates per-person rent, utility splits, and private room premiums correctly', () => {
      const res = calculateNomadColivingCostSharingIndex({
        monthlyRentUsd: 4000,
        numberOfNomads: 4,
        privateRoomPremiumRatio: 0.25,
        sharedUtilitiesUsd: 400
      });
      expect(res.valid).toBe(true);
      expect(res.totalMonthlyCost).toBe(4400);
      expect(res.basePerPersonRent).toBe(1000);
      expect(res.sharedUtilitiesPerPerson).toBe(100);
      expect(res.perPersonCostUsd).toBe(1100);
      expect(res.perPersonWithPremiumUsd).toBe(1350);
      expect(res.costEfficiencyScore).toBeGreaterThanOrEqual(50);
    });

    it('returns error for invalid non-positive monthly rent or number of nomads', () => {
      const invRent = calculateNomadColivingCostSharingIndex({ monthlyRentUsd: 0 });
      expect(invRent.valid).toBe(false);
      expect(invRent.error).toBe('Monthly rent and number of nomads must be positive numbers');

      const invNomads = calculateNomadColivingCostSharingIndex({ numberOfNomads: -2 });
      expect(invNomads.valid).toBe(false);
      expect(invNomads.error).toBe('Monthly rent and number of nomads must be positive numbers');
    });
  });

  describe('calculateNomadInternetSlaAndBackupRisk', () => {
    it('calculates internet uptime percentage, reliability score and tier accurately', () => {
      const res = calculateNomadInternetSlaAndBackupRisk({
      ...
      expect(res.valid).toBe(true);
      expect(res.uptimePercentage).toBe(99.72);
      expect(res.reliabilityScore).toBe(100);
      expect(res.riskTier).toBe('EXCELLENT_SLA');
    });

    it('returns error for negative primary speed input', () => {
      const inv = calculateNomadInternetSlaAndBackupRisk({ primarySpeedMbps: -10 });
      expect(inv.valid).toBe(false);
      expect(inv.error).toBe('Primary speed Mbps must be a non-negative number');
    });
  });

  describe('calculateNomadVisaAndTaxOptimizationIndex', () => {
    it('calculates visa qualification and safe tax risk level accurately', () => {
      const res = calculateNomadVisaAndTaxOptimizationIndex({
        monthlyIncomeUsd: 4500,
        stays: [
          { country: 'Spain', days: 90 },
      ...
      expect(res.valid).toBe(true);
      expect(res.visaIncomeStatusTier).toBe('QUALIFIED_EXCEEDS');
      expect(res.taxRiskLevel).toBe('LOW_SAFE');
      expect(res.optimizationScore).toBe(80);
    });

    it('returns error for non-positive monthly income or empty stays', () => {
      const invIncome = calculateNomadVisaAndTaxOptimizationIndex({ monthlyIncomeUsd: -100 });
      expect(invIncome.valid).toBe(false);
      expect(invIncome.error).toBe('Monthly income must be a positive number');
